...
std::unique_ptr<Point[]> GetPMJ02SamplesNoBalance(const int num_samples, random_gen& rng);

// Generates progressive multi-jittered (0,2) samples, but instead of
// ensuring (0,2) subsequences, it chooses subquadrants using the ox-plowing
// technique in Christensen et al.
std::unique_ptr<Point[]> GetPMJ02SamplesOxPlowing(const int num_samples, random_gen& rng);
